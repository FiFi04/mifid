...
public class IdentificationNumberValidator extends BaseValidator {
  ...
  protected static final int[] CARD_NUMBER_WEIGHTS = {2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2,
      1};

  @Override
  public boolean valid(String str, Field field) {
    boolean hasRightFormat = hasRightFormat(str,
        field.getAnnotation(Validate.class).maxTextLength());
    if (hasRightFormat) {
      int checksum = -1;
